...
if (html.includes("Workbench Mode") || html.includes("science.html")) pass("Workbench Mode link present"); else bad("Workbench Mode link missing");
if (!html.includes("scientific=!scientific")) pass("old Scientific Mode toggle behavior absent"); else bad("old Scientific Mode still toggles behavior");
...
if (data.length === 67) pass("dataset has 67 records"); else bad(`dataset has ${data.length} records`);
...
for (const d of data) {
  ...
  if (d.FirstReleaseYear !== null && d.FirstReleaseYear !== "" && (!Number.isInteger(d.FirstReleaseYear) || d.FirstReleaseYear < 1800 || d.FirstReleaseYear > 2100)) {
...
const nullYears = data.filter(d => d.FirstReleaseYear === null || d.FirstReleaseYear === "").length;
